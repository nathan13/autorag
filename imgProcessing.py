...
def WaitImage(image,limiar):
    from main import melhor_escala
    while True:
        imageloc = EncontrarImagem(image, limiar,melhor_escala)
        if imageloc:
            break

def EncontrarImagem(imagem, limi,melhor_escala):
...
def tirar_print(imagem, limiar, caminho_arquivo):
    """Procura por uma imagem na tela e tira um print da região com o mesmo tamanho da imagem encontrada."""
    posicao, largura_imagem, altura_imagem = encontrar_print(imagem, limiar)
...
def ler_texto_imagem(image):
    img = preprocessar_imagem(image)
